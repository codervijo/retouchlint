# AI Agent Context — retouchlint.com

## Summary

*one paragraph: what this site is, what it does*

RetouchLint helps real estate photographers, agents, and brokerages avoid disclosure mistakes around edited listing photos. Instead of unreliable AI-detection claims, it creates a clear workflow: pair original and final images, record edits like sky replacement, virtual staging, object removal, lawn enhancement, or color correction, collect photographer attestation, and generate disclosure language, original-photo links, and an audit packet.

## Audience

*one sentence: who this is for (broad demographic)*

Real estate photographers, listing agents, broker owners, and small brokerage compliance teams.
...
The first ICP is a real estate photographer or small media company serving agents in states with new or confusing photo-disclosure rules. They edit many listing shoots per month, are already being asked “what changed?”, and want a simple way to protect themselves and their agent clients without hiring legal help or building their own archive/attestation process.

## Goals

*1-2 sentences: primary business / product goal*

Start as the easiest way to create a defensible photo-edit disclosure packet for one listing. Win trust by being workflow-first, not AI-detector-first.
...
`proj=retouchlint.com`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=retouchlint.com` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=retouchlint.com` | Start the dev server, skipping install. |
| `make test proj=retouchlint.com` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=retouchlint.com
```
...
- **Deploy:** `wrangler deploy` (locally) or via Cloudflare's Git integration on push.
  ...
  `cd ../portfolio && make run ARGS="deploy retouchlint.com"` runs `gh repo create` and
...
Publish practical, state-specific and MLS-specific guides around listing photo disclosure, virtual staging rules, edited real estate photos, photographer attestation, original image access, and what counts as material alteration. Use SEO pages like “California real estate photo disclosure,” “virtual staging disclosure rules,” and “MLS edited photo compliance” to capture agents searching in panic.
...
`portfolio project check retouchlint.com` (run from `sites/portfolio/`).
