...
| **v1.B** | SEO content surface | 10 routes: pillar `/real-estate-photo-disclosure/`, `/faq/`, `/blog/` + 7 articles; shared `ArticleLayout.astro` with canonical/OG/Article+FAQPage+BreadcrumbList schema; `links.ts` resolves `linking-map.json` (24 of 44 links live, 20 dropped as dead targets) | ✅ |
| **v1.C** | editorial + legal verification | 13 statutory corrections against the enrolled text of AB 723 and nar.realtor; 2 fabricated quotations removed; 3 FAQ answers rewritten + 1 added; `noindex` and the sitemap filter lifted together | ✅ |
