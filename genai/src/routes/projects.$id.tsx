...
export const Route = createFileRoute("/projects/$id")({
  head: () => ({ meta: [{ title: "Listing — RetouchLint" }] }),
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <main className="container-page py-20 flex-1">
          ...
          <Link to="/dashboard" className="text-foreground underline mt-3 inline-block">← Back to dashboard</Link>
        </main>
        <SiteFooter />
      </div>
    );
      ...
      <main className="container-page py-10 flex-1">
        <div className="flex items-start justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => { if (confirm("Delete this listing?")) { store.remove(project.id); navigate({ to: "/dashboard" }); }}}
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          {project.published ? (
            <div className="mt-3 flex items-center justify-between">
              ...
              <Link to="/share/$id" params={{ id: project.id }} className="text-xs text-foreground underline">Open page →</Link>
            </div>
          ) : (
